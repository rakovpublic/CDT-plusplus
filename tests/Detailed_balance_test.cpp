...
TEST_CASE("4D detailed-balance acceptance ingredients are finite")
{
  ...
  auto const  probability =
      acceptance_probability(triangulation, proposal->triangulation,
                             move_tracker::MoveType4D::TWO_FOUR, couplings);
...
{
  auto        triangulation = FoliatedTriangulation4::periodic_seed(3);
  S4Couplings couplings{std::numeric_limits<long double>::infinity(), 0.2L,
                        0.1L, 36, 0.001L};
